...
class NgramVerifyInput(SpecInput):
    ...
    def verify(
        ...
        # Apply penalty - BALANCED version for maintaining performance
        if sampling_info.penalizer_orchestrator.is_required:
            # Use the optimized built-in penalty application for performance
            ...
            # Apply standard penalties efficiently
            sampling_info.apply_logits_bias(linear_penalty)
            
            # Only add minimal additional repetition control without performance impact
            reqs = sampling_info.penalizer_orchestrator.reqs()
            if reqs and hasattr(reqs[0].sampling_params, 'repetition_penalty'):
                repetition_penalty = reqs[0].sampling_params.repetition_penalty
                # Only apply light additional penalty if repetition_penalty is significantly greater than 1
                if repetition_penalty > 1.2:
                    for batch_idx, req in enumerate(reqs):
                        generated_tokens = req.output_ids
                        if generated_tokens:
                            # Create a simple frequency-based penalty (only for tokens appearing 5+ times)
                            token_counts = {}
                            for token in generated_tokens:
                                token_counts[token] = token_counts.get(token, 0) + 1
                            
                            # Only penalize tokens that appear very frequently (5+ times)
                            for token, count in token_counts.items():
                                if count >= 5:
                                    # Light additional penalty (0.1 per extra occurrence beyond 4)
                                    extra_penalty = (count - 4) * 0.1
                                    linear_penalty[batch_idx, token] -= extra_penalty
            
            # Apply penalties to all draft tokens
            expanded_penalty = torch.repeat_interleave(linear_penalty, self.draft_token_num, dim=0)
